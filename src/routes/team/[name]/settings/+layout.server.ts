...
export const load = (async ({ url, params: { name }, parent }) => {
	...
	const response = await supabase.from('teams').select<string, {
		...
		display_name: string
	}>('id, bio, name, roles:team_roles ( id, name, creator:users ( name, username ), position, created_at, permissions ), owner_id, avatar_url, created_at, website_url, display_name, members:team_members( id:user_id, role:team_roles!team_members_role_id_fkey ( permissions ) )').eq(isUUID(name) ? 'id' : 'name', name).order('position', { ascending: false, foreignTable: 'team_roles' }).limit(1).maybeSingle();
	if (response.error) {
